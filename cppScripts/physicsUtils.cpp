...
extern "C" {
    ...
    // @mite
    float simple_sum(const float* data, int length) {
        float sum = 0.0f;
    ...
    // @mite
    void apply_explosion_force(float* coordinatesX, float* coordinatesY, int totalObjects,
                               float targetX, float targetY, float radius, float force) {
    ...
    // @mite
    void heavy_math_transform(float* data, int length) {
        for (int i = 0; i < length; i++) {
            data[i] = std::sin(data[i]) * std::cos(data[i] * 0.5f)
                    + std::exp(-data[i] * 0.01f);
        }
    }
